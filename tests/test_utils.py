...
@pytest.mark.parametrize(
    ...
    [
        (2, 2, ("a", "b"), [(0, 1, dict(label="a")), (1, 2, dict(label="a")), (2, 0, dict(label="a")), (0, 3, dict(label="b")), (3, 4, dict(label="b")), (4, 0, dict(label="b"))]),
        (-1, -1, ("a", "b"), pytest.raises(Exception)),
    ]
)
def test_lift_labeled_two_cycles_graph_to_dot(tmp_path, first_cycle_size, second_cycle_size, labels, expected_edges):
    path_to_actual = tmp_path / "actual.dot"

    if isinstance(expected_edges, list):
        expected_graph = nx.DiGraph(expected_edges)
        lift_labeled_two_cycles_graph_to_dot(first_cycle_size, second_cycle_size, labels, path_to_actual)
